...
async def get_browser() -> Browser:
    ...
    if browser is None or not browser.is_connected():
        ...
        # Use channel="chromium" to get the full Chrome instead of headless-shell
...
async def scrape_url(
    ...
    try:
        ...
        # Wait the requested delay (simulates Steel's wait)
        if delay_ms > 0:
...
async def v1_create_session():
    ...
    b = await get_browser()
    context = await create_stealth_context(b)

    # Navigate to a neutral page to warm up the context
    page = await context.new_page()
...
def main():
    ...
    print("Endpoints: POST /v1/scrape, POST /v1/sessions, GET /v1/sessions/{{id}}/context")
    uvicorn.run(
